...
) -> tuple[utils.LocalFile, Optional[str]]:
  try:
    ...
    with tempfile.TemporaryDirectory(dir=tmp_parent_dir) as tmp_dir:
      ...
      if check_same_parse:
        ...
        errors = check_same_structure(game, upgraded_game)
        if errors:
          return local_file, 'different parse'
    ...
    return local_file, str(e)

  return local_file, None

def _monitor_results(
    results_iter: Iterator[tuple[utils.LocalFile, Optional[str]]],
    total_files: int,
    log_interval: int = 30,
):
  ...
  results: list[tuple[utils.LocalFile, Optional[str]]] = []

  for local_file, error in results_iter:
    if error is None:
      successful_conversions += 1
    else:
      last_error = local_file.name, error

    results.append((local_file, error))
...
def upgrade_archive(
...
) -> list[tuple[utils.ZipFile, Optional[str]]]:
  ...
  for zip_info in zf.filelist:
    ...
    if not zip_info.filename.endswith('.slp'):
      continue
