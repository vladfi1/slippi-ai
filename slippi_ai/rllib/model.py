...
class SlippiModel(TFModelV2):
  ...
  def forward(
    ...
    obs = input_dict['obs']
    obs = self.embed_game.from_nest(obs)

    # prev_action = input_dict['prev_action']
    # prev_action = self.embed_controller.from_nest(prev_action)

    # sar = embed.StateActionReward(obs, prev_action, input_dict['prev_reward'])
