...
def _make_converter(frame_skip, skip_delay, batch_size):
...
class FrameSkipConverterTest(unittest.TestCase):
  ...
  def _convert_rollouts(self, frame_skip, skip_delay, num_frames, num_rollouts, batch_size):
    converter = _make_converter(frame_skip, skip_delay, batch_size)
    # Reward for the transition into frame f is f.
    fake_rewards = lambda states, **kwargs: states[1:].astype(np.float32)
    with mock.patch.object(
        learner_lib.reward_lib, 'compute_rewards', fake_rewards):
      return [
