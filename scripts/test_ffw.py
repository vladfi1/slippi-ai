    ...
    try:
      ...
      for _ in range(env_config.rollout_length):
        ...
        for p in players:
          inputs[p] = utils.batch_nest_nt(
              [controller_lib.random_valid_controller() for _ in range(env_config.num_envs)])
        env.push(inputs)
      ...
      for i in range(env_config.rollout_length):
        ...
        for port, controller in inputs.items():
          if not controller_lib.controllers_equivalent(
            controller, output.gamestates[port].p0.controller):
            num_wrong += 1
...
def main(_):
  ...
  with open('test_ffw_results.json', 'w') as f:
    json.dump(results, f)
