...
def positional_encoding(seq_len, d_model, batch_size=1):
    ...
    num_angles = ceil(d_model/2)
    i_tensor = tf.expand_dims(tf.range(0, num_angles), 0) # [1, d_model/2]
    i_tensor = tf.broadcast_to(i_tensor, [seq_len, num_angles]) # [seq_len, d_model/2]
    j_tensor = tf.expand_dims(tf.range(0, seq_len), 1) # [seq_len, 1]
    j_tensor = tf.broadcast_to(j_tensor, [seq_len, num_angles]) # [seq_len, d_model/2]
    angles = encoding_angle(j_tensor, i_tensor) # [seq_len, d_model/2]

    # Apply sin to even indices, cos to odd indices
    num_cos = num_angles if num_angles == d_model//2 else d_model//2 # reminder goes to sin
    sins_angle = tf.math.sin(angles) # [seq_len, d_model/2]
    coss_angles = tf.math.cos(angles[:, 0:num_cos]) # [seq_len, d_model/2]
...
def attention_relative(queries, keys, values, relpos_k, relpos_v, masked=True, mem_size=None):
    """
    Returns the 'attention' between three sequences: keys, queries, and values
    Specifically this implementation uses 'scaled dot-product' attention.
    ...
    queries: (batch, seq_len, D_k)
    values: (batch, seq_len, D_v)
    relpos_k: (seq, seq, D_k)
    relpos_v: (seq, seq, D_v)
    mem_size: default is None, when set to positive integer will only consider mem_size prior elements for computation
    returns: (batch, seq_len, D_v)
    """
    if mem_size is not None:
    ...
    compat = tf.matmul(queries, keys, transpose_b=True) # [B, S, S]
    # Create second term
    queries_t = tf.transpose(queries, [1, 0, 2]) # [S, B, D_k]
    relpos_k_t = tf.transpose(relpos_k, [0, 2, 1]) # [S, D_k, S] is this right perm?
    q_relp_matmul = tf.matmul(queries_t, relpos_k_t) # [S, B, S]
    q_relp_matmul_t = tf.transpose(q_relp_matmul, [1, 0, 2]) # [B, S, S]
    adj_compat = compat + q_relp_matmul_t # [B, S, S]
    norm_compat = adj_compat / tf.sqrt(tf.cast(D_k, adj_compat.dtype)) # [B, S, S]
    if masked:
      i = tf.expand_dims(tf.range(S), 1)  # [S, 1]
      j = tf.expand_dims(tf.range(S), 0)  # [1, S]
      mask = i >= j # [S, S], mask[i, j] == i >= j
    ...
    att = tf.matmul(probs, values) # [B, S, D_V]
    # Create second term
    probs_t = tf.transpose(probs, [1, 0, 2]) # [S, B, S]
    probs_relv_matmul = tf.matmul(probs_t, relpos_v) # [S, B, D_v]
    probs_relv_matmul_t = tf.transpose(probs_relv_matmul, [1, 0, 2]) # [B, S, D_v]
    adj_att = att + probs_relv_matmul_t
    return adj_att
...
class MultiHeadAttentionBlock(snt.Module):
  def __init__(self, num_heads, output_size,  mem_size=None, name='MultiHeadAttentionBlock'):
    ...
    for _ in range(num_heads):
      ...
      self.W_K.append(snt.Linear(int(self.projection_size))) #output is d_model/num_heads
      self.W_V.append(snt.Linear(int(self.projection_size))) #output is d_model/num_heads
      self.W_Q.append(snt.Linear(int(self.projection_size))) #output is d_model/num_heads
    self.W_O = snt.Linear(output_size)
    self.l1_embed = snt.Linear(output_size)

  @snt.once
  def _initialize(self, inputs):
    input_seq = inputs.shape[1]
    S = input_seq + self.mem_size
    # Shared across heads
    self.relpos_k = tf.Variable(tf.zeros([S, S, self.projection_size]))
    self.relpos_v = tf.Variable(tf.zeros([S, S, self.projection_size]))
  ...
  def build_relpos(self, shape):
    size = shape[1] + self.mem_size
    rk = self.relpos_k[:size, :size, :]
    rv = self.relpos_v[:size, :size, :]
    return (rk, rv)
  ...
  def __call__(self, inputs, prev_state):
    ...
    for i in range(self.num_heads):
      ...
      rk, rv = self.build_relpos(inputs.shape)
      head_i = attention_relative(queries, keys, values, rk, rv, mem_size=M) # [B, M+S, D_m/h]
      heads.append(head_i)
    # MHA(Q, K, V) <- Concat(head_1...head_h)W^O
    proj_heads = self.W_O(tf.concat(heads, -1))  # [B, M+S, D_m]
    multi_head = proj_heads[:, M:] # [B, S, D_m]
    # TODO technically we don't need to track queries in memory  
...
class EncoderOnlyTransformer(snt.Module):
  ...
  def __call__(self, inputs, prev_state):
    ...
    x = tf.transpose(inputs, [1, 0, 2]) # [B, T, O]
    # Disabled to favor relative encoding, this could be on flag
